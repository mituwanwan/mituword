...
export default function EditProjectPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        const res = await fetch(`/api/projects?search=${params.id}`);
        if (res.ok) {
          const data = await res.json();
          const project = Array.isArray(data) ? data.find((p: any) => p.id === params.id) : null;
          if (project) {
            setForm({
              title: project.title || '',
              slug: project.slug || '',
  ...
  if (loading) {
    return <div className="p-8 text-center dark:text-void-dust/50 text-realm-mist/50">加载中...</div>;
  }
